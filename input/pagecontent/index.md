...
The content in this implementation guide is provided for reuse and proposed to help streamline implementation and development of CQL-based knowledge artifacts in the US Realm. The guidance here is published informative and the IG imposes no conformance expectations on systems that make use of it. This does not mean that the content cannot be used in production; the content in this implementation guide is stable as of publication versions, and will use semantic versioning to communicate change impact across publication versions, as described in the [CRMI IG versioning](https://hl7.org/fhir/uv/crmi/artifact-lifecycle.html#artifact-versioning) topic. In particular, this means that production systems can expect no breaking changes without a corresponding major version increment, and that any minor version increment indicates compatible changes.
